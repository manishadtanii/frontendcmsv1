...
const Dashboard = () => {
  ...
  const handleUpdateSection = async (updatedData) => {
    ...
    Object.keys(content).forEach(key => {
      const value = (typeof content[key] === 'object' && content[key] !== null) 
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-hidden">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button onClick={handleToggleContacts} className={`px-4 py-2 rounded-full ${showContacts ? 'bg-cyan-500 text-black' : 'bg-gray-800'}`}>Contacts</button>
            <button onClick={handleToggleUsers} className={`px-4 py-2 rounded-full ${showUsers ? 'bg-cyan-500 text-black' : 'bg-gray-800'}`}>Users</button>
          </div>
        </div>
        <div className="flex-1 bg-[#0a0f14]/80 border border-gray-800 rounded-[40px] p-10 overflow-y-auto custom-scrollbar">
                  ...
                  <tbody>
                    {contacts.map(c => (
                      <tr key={c._id} className="border-t border-gray-800">
                        <td className="p-2">{c.firstName || c.contactName || '-'}</td>
                        <td className="p-2">{c.contactEmail || '-'}</td>
                        <td className="p-2">{c.contactNumber || '-'}</td>
                        <td className="p-2">{(c.servicesNeeded && c.servicesNeeded.length) ? c.servicesNeeded.join(', ') : '-'}</td>
                        <td className="p-2">{c.preferredDate ? (new Date(c.preferredDate).toLocaleDateString ? new Date(c.preferredDate).toLocaleDateString() : c.preferredDate) : '-'}</td>
                        <td className="p-2">{c.createdAt ? new Date(c.createdAt).toLocaleString() : '-'}</td>
                        <td className="p-2">
                          <select defaultValue={c.status} onChange={(e) => handleUpdateContactStatus(c._id, e.target.value)} className="bg-black text-white text-sm rounded px-2 py-1">
                        ...
                        <td className="p-2 flex gap-2">
                          <button onClick={() => openEditContact(c)} className="px-3 py-1 rounded bg-yellow-400 text-black font-medium hover:shadow-md">Update</button>
                          <button onClick={() => handleDeleteContact(c._id)} className="px-3 py-1 rounded bg-red-600 hover:bg-red-700">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {showEditModal && editContact && (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              <div className="absolute inset-0 bg-black/60" onClick={closeEditModal}></div>
              <div className="bg-[#0b1318] rounded-2xl p-6 z-10 w-full max-w-2xl">
                <h3 className="text-xl font-bold mb-4">Edit Contact</h3>
                <div className="grid grid-cols-2 gap-4">
                  <input value={editContact.firstName || ''} onChange={(e) => setEditContact(prev => ({ ...prev, firstName: e.target.value }))} className="p-2 bg-transparent border rounded" placeholder="First Name" />
                  <input value={editContact.contactName || ''} onChange={(e) => setEditContact(prev => ({ ...prev, contactName: e.target.value }))} className="p-2 bg-transparent border rounded" placeholder="Contact Name" />
                  <input value={editContact.contactEmail || ''} onChange={(e) => setEditContact(prev => ({ ...prev, contactEmail: e.target.value }))} className="p-2 bg-transparent border rounded col-span-2" placeholder="Email" />
                  <input value={editContact.contactNumber || ''} onChange={(e) => setEditContact(prev => ({ ...prev, contactNumber: e.target.value }))} className="p-2 bg-transparent border rounded" placeholder="Phone" />
                  <input value={(Array.isArray(editContact.servicesNeeded) ? editContact.servicesNeeded.join(', ') : (editContact.servicesNeeded || ''))} onChange={(e) => setEditContact(prev => ({ ...prev, servicesNeeded: e.target.value }))} className="p-2 bg-transparent border rounded col-span-2" placeholder="Services (comma separated)" />
                  <select value={editContact.status || 'new'} onChange={(e) => setEditContact(prev => ({ ...prev, status: e.target.value }))} className="p-2 bg-transparent border rounded">
                    <option value="new">new</option>
                    <option value="pending">pending</option>
                    <option value="closed">closed</option>
                  </select>
                  <textarea value={editContact.notes || ''} onChange={(e) => setEditContact(prev => ({ ...prev, notes: e.target.value }))} className="p-2 bg-transparent border rounded col-span-2" placeholder="Notes" />
                </div>
                <div className="mt-4 flex justify-end gap-3">
                  <button onClick={closeEditModal} className="px-4 py-2 rounded bg-gray-800">Cancel</button>
                  <button onClick={handleSaveContact} className="px-4 py-2 rounded bg-cyan-500 text-black font-bold">Save</button>
                </div>
              </div>
            </div>
          )}
